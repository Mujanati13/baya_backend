...
router.get("/product-performance/:month?", asyncHandler(async (req, res) => {
    ...
    return new Promise((resolve, reject) => {
        db.query(sql, queryParams, (err, results) => {
            ...
            resolve(res.json({
                ...
                products: products,
                summary: {
                    total_products_sold: products.reduce((sum, product) => sum + product.quantity_sold, 0),
...
router.get("/product-sales-stats", asyncHandler(async (req, res) => {
    const sql = `
    SELECT 
      a.ID_ART,
  ...
  `;
    ...
    return new Promise((resolve, reject) => {
        db.query(sql, (err, results) => {
            ...
            const summary = {
                total_products_sold: results.reduce((sum, product) => sum + product.units_sold, 0),
                total_revenue: results.reduce((sum, product) => sum + product.total_revenue, 0),
                average_order_value: results.reduce((sum, product) => sum + product.total_revenue, 0) /
                    results.reduce((sum, product) => sum + product.number_of_orders, 0) || 0,
                best_selling_product: results.length > 0 ?
                    {
